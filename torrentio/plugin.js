(function () {
    ...
    function buildStreamLabel(title, name) {
        ...
        var tags     = (title.match(/(2160p|1080p|720p|WEBRip|WEB-DL|x265|x264|10bit|HEVC|H264)/gi) || [])
    ...
    function getQuality(title) {
        return (title.match(/(2160p|1080p|720p)/i) || [])[1] || 'Unknown';
    }
    ...
    async function torrentioLoadStreams(url, cb) {
        try {
            ...
            res.streams.forEach(function(s) {
                var source  = (s.name || 'Torrentio').split('\n')[0].trim();
                var label   = buildStreamLabel(s.title || '', s.name || '');
    ...
    function buildMagnetLabel(title, prefix) {
        var tags     = (title.match(/(2160p|1080p|720p|WEBRip|WEB-DL|x265|x264|10bit|HEVC|H264)/gi) || [])
                       .map(function(t) { return t.toUpperCase(); })
                       .filter(function(t, i, a) { return a.indexOf(t) === i; })
                       .join(' | ');
        var seeder   = (title.match(/👤\s*(\d+)/) || [])[1] || '0';
        var provider = ((title.match(/⚙️\s*([^\n]+)/) || [])[1] || 'Unknown').trim();
        return prefix + ' | ' + (tags ? tags + ' | ' : '') + 'Seeder: ' + seeder + ' | Provider: ' + provider;
    }
    ...
    async function torrentioLoadStreams(url, cb) {
        try {
            ...
            var imdbId  = data.imdbId;
            var isMovie = data.type === 'movie';
            if (!imdbId) return cb({ success: false, error: 'IMDB ID tidak ditemukan.' });
